...
if (~isempty(overlaydataid) && (data_only == 'n'))
    ...
    for n = 1:nmax,
        ...
        fprintf(fid, '%d %d %d %8.3f %8.3f %8.3f %8.6e %s \n', loc_i, loc_j, loc_k, overlaydata(n,1), overlaydata(n,2), -overlaydata(n,3), data_vector_2(n), overlaylabel(n,:));
    end
    fclose(fid);
...
if (plot_secondary == 'y'),
    ...
    if ( ~isempty(dataid_2) || ~isempty(overlaydataid) ),
        ...
        %
        plot_crossplotc(loc_x_data,loc_y_data,loc_D_data,loc_x_label,loc_y_label,loc_D_label,POPT,[par_pathout '/' filename '.CROSSPLOT']);
        %
    end
    %
